...
router.post('/collect', async (req, res) => {
    try {
        ...
        // 필수 필드 검증
        if (!event_name || !timestamp || !client_id || !session_id) {
            return res.status(400).json({
                error: '필수 필드가 누락되었습니다.',
                required: ['event_name', 'timestamp', 'client_id', 'session_id']
            });
        }

        // PostgreSQL 함수를 사용하여 이벤트 저장
        const query = `
            SELECT insert_analytics_event(
        ...
        const values = [
            ...
            context?.properties ? JSON.stringify(context.properties) : null,
            context?.device ? JSON.stringify(context.device) : null,
            context?.geo ? JSON.stringify(context.geo) : null,
            context?.traffic_source ? JSON.stringify(context.traffic_source) : null
        ];
...
router.get('/events', async (req, res) => {
    try {
        ...
        let query = `
            SELECT * FROM analytics_events_full 
            WHERE 1=1
        `;
        const values = [];
        let valueIndex = 1;
        ...
        if (start_date) {
            ...
            values.push(parseInt(start_date));
        }

        if (end_date) {
            query += ` AND timestamp <= $${valueIndex++}`;
            values.push(parseInt(end_date));
        }
...
router.get('/stats', async (req, res) => {
    try {
        ...
        if (start_date && end_date) {
            ...
            values.push(parseInt(start_date), parseInt(end_date));
        }
        ...
        const deviceStatsQuery = `
            SELECT 
                ...
                COUNT(*) as count
            FROM analytics_events e
            JOIN analytics_device_info d ON e.id = d.event_id
            ${whereClause}
