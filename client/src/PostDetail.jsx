  ...
  return (
    ...
    >
      <div style={{ textAlign: "left" }}>
        <Link to="/" onClick={handleBackToListClick}>
          <small>← 목록으로 돌아가기</small>
        </Link>
      </div>
      <div
        style={{
      ...
      >
        ...
        >
          <h2 style={{ marginBottom: 0, whiteSpace: "nowrap" }}>
            {post.title}
          </h2>
          <p
          ...
          >
            <strong>작성자:</strong> {post.author} | <strong>작성일:</strong>{" "}
            {new Date(post.created_at).toLocaleString()}
          </p>
        </div>
        {isAuthor && (
          <div style={{ display: "flex", flexDirection: "row", gap: "0.5rem" }}>
            <Link to={`/posts/${post.id}/edit`} onClick={handleEditPostClick}>
              <button style={{ whiteSpace: "nowrap" }}>게시글 수정</button>
            </Link>
            <button style={{ whiteSpace: "nowrap" }} onClick={handleDeletePost}>
              게시글 삭제
            </button>
          </div>
        )}
            ...
            {editingCommentId === c.id ? (
              ...
              <>
                ...
                {c.author_id !== authorId && isPostAuthor && (
                  <div style={{ textAlign: "right" }}>
                    <button onClick={() => handleDeleteComments(c.id)}>
                      삭제
                    </button>
                  </div>
                )}
